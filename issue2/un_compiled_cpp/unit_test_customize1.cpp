...
int main() {
  ...
  // float init[4] = {0.1, 0.2, 0.3, 0.4};
  auto init = std::initializer_list<float>({0.1, 0.2, 0.3, 0.4});
  std::copy(init.begin(), init.end(), *reinterpret_cast<float(*)[4]>(data));
  
  // *reinterpret_cast<float*>(data) = 0.4;
  // std::cout << "data_type " << typeid(data).name() << std::endl;
  // std::cout << *reinterpret_cast<float*>(data) << std::endl;

  checkedMXCall(MXNDArrayGetData, in_arr_2, &data);
  init = std::initializer_list<float>({0.5, 0.6, 0.7, 0.8});
  std::copy(init.begin(), init.end(), *reinterpret_cast<float(*)[4]>(data));
  ...
  *reinterpret_cast<float*>(data) = 0;
  ...
  uint32_t grad_req_1[1] = {1}; //{0} 
  uint32_t grad_req_2[1] = {1}; //{1}
  // 4
...
// elementwise_add, not called by python?
  ...
  // *reinterpret_cast<float*>(data) = 7.0;
  init = std::initializer_list<float>({70, 8, 9, 10});
  std::copy(init.begin(), init.end(), *reinterpret_cast<float(*)[4]>(data));
