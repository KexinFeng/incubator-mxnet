...
int main(int argc, char** argv) {
  uint32_t req1 = std::stoi(argv[1]);
  uint32_t req2 = std::stoi(argv[2]);
  // req1 = 0;
  // req2 = 1;
  ...
  // const char json[] = "{\"nodes\": [{\"op\":\"null\",\"name\":\".Inputs.Input1\",\"inputs\":[]},{\"op\":\"null\",\"name\":\".Inputs.Input2\",\"inputs\":[]},{\"op\":\"elemwise_mul\",\"name\":\".$0\",\"inputs\":[[0,0,0],[1,0,0]]},{\"op\":\"_copy\",\"name\":\".Outputs.Output\",\"inputs\":[[2,0,0]]}],\"arg_nodes\":[0,1],\"heads\":[[3,0,0]]}";
  SymbolHandle sym;
  ...
  *reinterpret_cast<float*>(data) = 0;
  ...
  uint32_t grad_req_1[1] = {req1}; //{0} 
  uint32_t grad_req_2[1] = {req2}; //{1}
  // 4
...
// elementwise_add, not called by python?

  // 5
  /* Create cached op */
  const char *cachedop_keys[1] = {"static_alloc"};
  const char *cachedop_vals[1] = {"true"}; // false will avoid triggering the bug
...
// 7
  ...
  *reinterpret_cast<float*>(data) = 2021.0;
